...
for row in range(R) :
    if arr[row][0] == -1 :
        air1, air2 = row, row+1         # 공기청정기 위치 저장
        arr[row][0] = arr[row+1][0] = 0   # 계산 편의를 위해 0으로 설정
        break
    ...
    for r in range(R) :
        for c in range(C) :
            if arr[r][c] > 4:       # 5보다 작으면 몫이 0이므로 확산이 일어나지 않음
                ...
                for i in range(4) :
                    nr, nc = r + dr[i], c + dc[i]
                    # 범위 내이고, 공기청정기가 아니라면 미세먼지 확산
                    if 0 <= nr < R and 0<= nc <C and (nr,nc) != (air1,0) and (nr,nc) != (air2,0) :
                        temp[r][c] -= t
                        temp[nr][nc] += t
